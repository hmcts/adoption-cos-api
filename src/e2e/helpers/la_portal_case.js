...
const laPortalCase = async (caseId) => {
    ...
    //const url = "https://ccd-data-store-api-adoption-cos-api-pr-511.service.core-compute-preview.internal";

    const url = "http://localhost:4452";
    ...
    try {
        ...
        let s2sAuthRequest = await Axios.post(`${s2sUrl}/lease`, { "microservice": "adoption_web", oneTimePassword });

        s2sAuth = s2sAuthRequest.data;

        let idamData = `username=adoption.systemuser@mailinator.com&password=Hmcts@123&client_id=${clientId}&client_secret=${idamSecret}&grant_type=password&redirect_uri=${callbackUrl}&scope=openid%20profile%20roles`;
...
createCompleteCase = async () => {
console.log("creating cui case")

  const caseId = await cui.createCase();

  console.log("created case: {}", caseId);

  console.log("creating laportal case");
