...
class CaseworkerAmendOtherPartiesDetailsTest {
    ...
    @Test
    void caseworkerAmendOtherPartiesEventaboutToStartTest() {

        var caseDetails = getCaseDetails();
        final var instant = Instant.now();
        final var zoneId = ZoneId.systemDefault();
        final var expectedDate = LocalDate.ofInstant(instant, zoneId);
        var result = caseworkerAmendOtherPartiesDetails.aboutToStart(caseDetails);
        assertThat(result.getData().getAdopAgencyOrLA().getAdopAgencyAddress()).isNotNull();
