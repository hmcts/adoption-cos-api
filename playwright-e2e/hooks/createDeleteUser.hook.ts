import { createCaseworkerUser, deleteCaseworkerUser, getAccessToken } from '../helpers/idamTestApiHelpers';
...
export async function setupUser(): Promise<{ email: string; password: string; id: string }> {
  try {
    ...
    const user = await createCaseworkerUser(token);
...
export async function teardownUser(userId: string): Promise<void> {
  ...
  if (token) {
    try {
      await deleteCaseworkerUser(token, userId);
