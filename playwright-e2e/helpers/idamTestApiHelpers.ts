 ...
 */
export async function createCaseworkerUser(token: string): Promise<{ email: string; password: string; id: string }> {
  if (!process.env.IDAM_CASEWORKER_USER_PASSWORD) {
    throw new Error('PASSWORD environment variable is not defined');
  }
  const uniqueId = uuidv4();
  const id = uniqueId;
  const password = process.env.IDAM_CASEWORKER_USER_PASSWORD as string;
  const email = `TEST_ADOPTION_USER_caseworker-user.${uniqueId}@test.local`;
 ...
 */
export async function deleteCaseworkerUser(token: string, id: string): Promise<void> {
